...
```{r}
# Read in data
setwd("C:/Users/chris/OneDrive/Desktop/School/linearmodelsfinal")
...
```{r}
drs4 <- drs3[ ,-c(2:4,8,11,13)]
```

```{r}
logit_model2<-glm(condition~., drs4, family='binomial')
...
# Seeing multicollinearity between log fam size and family, which makes sense.
```
...
```
```{r}
prediction<-mutate(drs4, prob=predict(logit_model2, type="response"),odds=log(prob/(1-prob)),classify=ifelse(prob>.5,1,0))

table(prediction$classify,drs4$condition)
```
```{r}
...
```{r}
# Remove log values and health column
drs3 <- drs2[ ,-c(2,4:5,8:10)]
```

```{r}
# Create model
logit1 <- glm(condition~., drs3, family='binomial')
summary(logit1)
```

```{r}
# Remove idp, sex, and family
drs4 <- drs3[ , -c(2,7,14)]

# 2nd logit model
logit2 <- glm(condition~., drs4, family='binomial')
summary(logit2)
```

```{r}
predict(logit2, data.frame(visits = 2, physlim = 0, num_disease = 4, schooling = 12,
                           age = 35, black = 0, payment = 226, deductible = 2120,
                           income = 2500, child = 0, insurance = 95),
                           type="response")

```

```{r}
predicted_probs <- predict(logit2, type = "response")
predicted_classes <- ifelse(predicted_probs > 0.5, 1, 0)
confusion_matrix <- table(Predicted = predicted_classes, Actual = drs4$condition)
confusion_matrix
```
...
accuracy <- (TP + TN) / (TP + TN + FP + FN) # [1] 0.9144457
TPR <- TP / (TP + FN)  # Sensitivity, 0.9908317
TNR <- TN / (TN + FP)  # Specificity 0.1627282
FPR <- FP / (FP + TN) # 0.8372718
FNR <- FN / (FN + TP) # 0.009168304
# High false positive rate but also high true positive rate
```